...
\usepackage{graphicx}
\usepackage{ntheorem}
\usepackage{bbm} % per avere \mathbbm{1}
...
% ACHTUNG ACHTUNG: Quando contribuite a fare modifiche al file .tex assicuratevi che compili e compilate come pdf!!
% Ciò significa che, se siete su linux, prima di fare git commit date i comandi:
% pdflatex natale_rapp.tex
% Assicurarsi che non ci siano errori e guardare la parte che avete modificato nel pdf per vedere che sia venuta bene
% (A vostra scelta, tanto non entrano nel repository) rm *.aux *.log *.thm
% Commitare le modifiche e fare il push
% Grazie, Nazi Balbo
